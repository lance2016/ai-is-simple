...
**本章新增：** 新增 `run_workflow`，并用 `Journal`（运行记录）保存每一步的结果，方便中断后接着运行。

Agent Loop 适合探索：模型看见结果后再决定下一步。Workflow（工作流）把步骤提前写好，适合代码审查、资料汇总、发布检查这类流程固定的任务。

## 先看图

- 模型只需要发起一次 `run_workflow`；
- 固定的 Workflow 代码负责安排并行检查和汇总；
- 每个步骤的结果写进运行记录（`Journal`）；
...
并行步骤由 Python 程序提前写好。模型可以判断每一步的结果，但不能把既定流程替换成任意代码。
