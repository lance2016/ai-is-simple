...
- 实战 Agent 必须保留基础聊天能力：问候、闲聊和不需要访问工作区的概念提问不应调用工具。这一点通过 system prompt 约束模型即可，不需要在 Harness 层额外做意图分流。
- 当实战需要展示 tool call、工具返回和权限请求时，优先提供本地网页界面；界面只展示状态并提交用户决定，真正的路径限制、命令检查和权限判断仍必须在 Python Harness 中完成。
...
- 改完并检查通过后直接提交到 `main`，不用等作者要求；推送到远程要等作者明确要求。
...
- Agent Loop 配图必须画出终止分支：当模型不再调用工具时，进入“输出答案 / 结束”；不能只画一个没有出口的循环。
...
- 学习路线分成“核心主干（00～03、15）”和“按场景选用的可选能力”，不要把 18 章写成必须全部具备的线性递进。
...
**本章新增：** 一句话说明在基础循环上多了什么（主干第 00 章可省略）
...
## 想一想
一个简单问题 + 折叠的参考思路（<details>，两三句话，不写成面试题库）
```
