...
- 实战 Agent 必须保留基础聊天能力：问候、闲聊和不需要访问工作区的概念提问不应进入工具循环；可以在 Harness 层做轻量意图分流，只有工作区任务才把工具定义发给模型。
- 当实战需要展示 tool call、工具返回和权限请求时，优先提供本地网页界面；界面只展示状态并提交用户决定，真正的路径限制、命令检查和权限判断仍必须在 Python Harness 中完成。
- 工具调用优先使用 OpenAI 兼容接口返回的原生 `message.tool_calls`；如果兼容层返回 DSML 文本，程序可以做兼容转换，但不能把 DSML 原文当作用户可见的最终结果。
